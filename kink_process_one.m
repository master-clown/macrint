...
%--------------------------------------------------------------------------
    ...
    if(mode == 4)
        [sif_mat, avg_tr] = kink_process_one_mode4(sif_type, phi, yoff, chl, a, N, bst_lst);
        return;
    end

    num_rect_row = 4 + 4*N;                                                         % number of cracks in a row or col (or num of cells along them)
   %num_rect     = num_rect_row^2;
    num_cr_row   = num_rect_row + 1;                                                % number of crack rows or col
...
function [sif_mat, avg_tr] = kink_process_one_mode4(sif_type, phi, yoff, chl, a, N, bst_lst)
% mode4 specialization

    num_rect_row = 1;
   %num_rect     = num_rect_row^2;
    num_cr_row   = 4 + 4*N;                                                         % number of cracks in a row
    num_cr_hv    = num_rect_row*num_cr_row;                                         % total number of horizontal (or vertical) cracks
    num_cr       = num_cr_hv;                                                       % total number of cracks
    
    cc_lst = zeros(num_cr, 2);                                                      % cracks' centers list    
    hl_lst = chl * ones(num_cr, 1);
    
    for i_col = 1:num_cr_row
        x0 = a*(i_col - (num_cr_row+1)/2);
        y0 = 0.0;
    ...
    ind_lst = [ ...                                                         %%%%%%%%%%%%%%%% horizontal cracks 
                ...
                num_cr_row/2 - 1, ...                                       % (-2, 0)
                num_cr_row/2 + 0, ...                                       % (-1, 0)
                num_cr_row/2 + 1, ...                                       % (+1, 0)   -- the one which is rotated
                num_cr_row/2 + 2, ...                                       % (+2, 0)
                num_cr_row        ...                                       % (+end, 0)             
              ];
